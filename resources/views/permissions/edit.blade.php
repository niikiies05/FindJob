<div class="modal fade" id="permissionEdit{{ $permission->id }}" tabindex="-1" role="dialog" aria-labelledby="addUserLabel"
    aria-hidden="true">
        ...
        <div class="modal-content">
            <form action="{{ route('permissions.update', $permission->id) }}" method="post">
                ...
                <div class="modal-header d-flex align-items-center bg-dark text-white">
                    <h6 class="modal-title mb-0" id="addUserLabel">{{ __("permission.edit_page_title") }}</h6>
...
<script>
    $(document).ready(function() {
        @if (count($errors) > 0)
            ...
            // keyboards:false
            // });
            $('#permissionEdit{{!! $permission->id !!}}').modal('show');
        @endif
    });
</script>
