    ...
    <div class="main-body">
        ...
        {{-- @can('permission-create') --}}
            <div class="list-with-gap">
                <button class="btn btn-primary has-icon" type="button" data-toggle="modal" data-target="#addSalary"><i
                        class="material-icons mr-2">add_circle_outline</i>Add salary type</button>
            </div>
        {{-- @endcan --}}
        @include('salary_type.create')
                    ...
                    <tbody>
                        @foreach ($salaries as $salary)
                            <tr>
                                <td>{{ ++$i }}</td>
                                <td>{{ $salary->name }}
                                    {{-- @foreach ($salary->roles as $role)
                                        {{ $role->name }}
                                    @endforeach --}}
                                </td>
                                <td>
                                    <form action="{{ route('salaryTypes.destroy', $salary->id) }}" method="POST">

                                        {{-- @can('salary-edit') --}}
                                            <a class="btn btn-link btn-icon bigger-130 text-success" data-toggle="modal"
                                                data-target="#salaryEdit{{ $salary->id }}" title="Edit"
                                                href="{{ route('salaryTypes.edit', $salary->id) }}"><i
                                        ...
                                        {{-- @can('salary-delete') --}}
                                            <button type="submit" class="btn btn-link btn-icon bigger-130 text-danger"
                                                title="Delete" onclick="return confirm('{{ __('salary.delete_alert') }}');">
